  ...
  const checks = [
    ...
    runCheck("prompt includes research note constraints", () => {
      const prompt = buildCodexExecPrompt(
        "C:/notes/note.md",
        [{ title: "驥丞ｭ舌ヵ繝ｼ繝ｪ繧ｨ螟画鋤", lineNumber: 12, isEmpty: true }],
        {
          mode: "research",
          fillPolicy: "emptyOnly",
          researchField: "驥丞ｭ舌さ繝ｳ繝斐Η繝ｼ繧ｿ",
          outputLanguage: "Japanese",
          noteStyle: "隱ｬ譏・谿ｵ關ｽ + 蜿り・枚迪ｮ1譛ｬ"
        }
      );

      assert.match(prompt, /驥丞ｭ舌ヵ繝ｼ繝ｪ繧ｨ螟画鋤 \(line 12\)/);
      assert.match(prompt, /Research field: 驥丞ｭ舌さ繝ｳ繝斐Η繝ｼ繧ｿ/);
    ...
    runCheck("manifest contributes shortcut and self-test command", () => {
      ...
      assert.equal(commands.includes("codexNoteHelper.runSelfTest"), true);
      ...
      assert.equal(
        properties["codexNoteHelper.showCodexProgress"].default,
        true
      );
